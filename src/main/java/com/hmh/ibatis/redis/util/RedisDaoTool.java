package com.hmh.ibatis.redis.util;
 ...
 * 
 */
public class RedisDaoTool extends AbstractBaseRedisDao<String, Object> {
	...
	public boolean addObject(final String key, final Object object,
			final Long timeout, Class<?> clazz) throws Exception {
	...
	public boolean addString(final String key, final String value,
		...
		boolean result = redisTemplate.execute(new RedisCallback<Boolean>() {
			public Boolean doInRedis(RedisConnection connection)
				...
				Boolean result = connection.setNX(key.getBytes(),
						value.getBytes());
				if (result == false)
					return result;
	 ...
	 * @return true 操作成功，false 已存在值
	 */
	public Boolean addSet(final String key, final String value, final Long timeout) {
		Boolean result = redisTemplate.execute(new RedisCallback<Boolean>() {
			public Boolean doInRedis(RedisConnection connection)
					throws DataAccessException {
				Boolean result = connection.sAdd(key.getBytes(), value.getBytes());
				if (result)
					return result;
				if (timeout != null && timeout > 0)
					connection.expire(key.getBytes(), timeout);
